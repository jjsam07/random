...
void _procmem_write(mach_port_t task, vm_address_t address, void *buffer, size_t size) {
	...
	vm_region_basic_info_data_t newinfo;
	vm_size_t vmsize;
	...
	if (!(info.protection & VM_PROT_WRITE)) {
		if (debug) printf("procmem_write: vm_protect: writing to non-writable region, preparing for writing\n"); // Debug output
		res = vm_protect(task, address, size, NO, info.protection | VM_PROT_WRITE);
		if (res != KERN_SUCCESS) {
			printf("procmem_write: vm_protect: %s\n", mach_error_string(res));
			exit(0);
		}
		res = vm_region(task, &region_address, &vmsize, VM_REGION_BASIC_INFO, (vm_region_info_t)&newinfo, &info_count, &object);
		if (res != KERN_SUCCESS) {
			printf("procmem_write: vm_region: %s\n", mach_error_string(res));
			exit(0);
		}
		if (debug) printf("new region protection: %c%c%c\n", (newinfo.protection & VM_PROT_READ) ? 'r':'-', (newinfo.protection & VM_PROT_WRITE) ? 'w':'-', (newinfo.protection & VM_PROT_EXECUTE) ? 'x':'-'); // Debug output
	}
	...
	if (info.protection & VM_PROT_EXECUTE) {
		if (!no_flush) {
			printf("Flushing and invalidating data as we are writing to executable region.\n");
